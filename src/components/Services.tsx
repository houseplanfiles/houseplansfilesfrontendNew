        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-4 sm:gap-5">
            {HOME_SERVICES.map((option) => (
              ...
              >
                <div className={`w-16 h-16 rounded-xl flex items-center justify-center mb-3 transition-colors duration-300 ${option.color.split(" ")[0]} ${option.color.split(" ")[1]}`}>
                  <option.icon className="w-8 h-8 transition-transform duration-300 group-hover:scale-110" />
                </div>
                <span className="text-lg md:text-xl font-extrabold text-gray-800 group-hover:text-orange-600 transition-colors px-1">
                  {option.label}
                </span>
              </Link>
            ))}
