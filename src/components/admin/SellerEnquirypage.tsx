...
import { Loader2, ServerCrash, Eye, Inbox, X, Save, User, Package as ProductIcon, Store } from "lucide-react";
...
const InquiryDetailModal = ({ inquiryId, onClose, onUpdate }: InquiryDetailModalProps) => {
  const dispatch = useDispatch<AppDispatch>();
  const { selectedInquiry, actionStatus } = useSelector((state: RootState) => state.sellerInquiries);
  const [status, setStatus] = useState("");

  useEffect(() => {
    if (inquiryId) dispatch(fetchInquiryById(inquiryId));
  }, [inquiryId, dispatch]);
  ...
  const isLoading = actionStatus === "loading" || selectedInquiry?._id !== inquiryId;
        ...
        <div className="p-8">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              <h3 className="font-bold text-gray-700 flex items-center gap-2"><User size={16} /> Customer Info</h3>
              <p><span className="font-semibold text-gray-500">Name:</span> {selectedInquiry.name}</p>
              <p><span className="font-semibold text-gray-500">Email:</span> {selectedInquiry.email}</p>
              <p><span className="font-semibold text-gray-500">Phone:</span> {selectedInquiry.phone}</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              <h3 className="font-bold text-gray-700 flex items-center gap-2"><ProductIcon size={16} /> Product Info</h3>
              <p><span className="font-semibold text-gray-500">Product:</span> {selectedInquiry.product?.name}</p>
              <p><span className="font-semibold text-gray-500">Date:</span> {new Date(selectedInquiry.createdAt).toLocaleString()}</p>
              <div className="flex items-center gap-2 pt-1">
                <Store size={16} className="text-gray-500" />
                <div>
                  <p className="text-xs text-gray-400">Seller</p>
                  <p className="font-semibold">{selectedInquiry.seller?.businessName}</p>
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-2 border-t pt-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <Button onClick={handleStatusUpdate} disabled={actionStatus === "loading"} className="h-11">
...
const SellerEnquiryPage = () => {
  ...
  const loadInquiries = () => dispatch(fetchAllInquiries());
  ...
  if (listStatus === "failed") return (
    <div className="text-center py-20">
      <ServerCrash className="mx-auto h-12 w-12 text-red-400" />
      <h3 className="mt-2 font-semibold text-gray-700">Failed to load inquiries.</h3>
    ...
    <div className="p-4 md:p-6 space-y-6">
            ...
            {inquiries.length > 0 ? inquiries.map(inquiry => (
              <tr key={inquiry._id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4 text-right">
                  <Button variant="ghost" size="sm" onClick={() => handleViewDetails(inquiry._id)}>
              ...
              <tr>
                <td colSpan={6} className="text-center p-16 text-gray-500">
                  ...
                  <p className="mt-2 font-semibold">No inquiries found in the system.</p>
                </td>
              </tr>
            )}
          </tbody>
      ...
      {isModalOpen && selectedInquiryId && (
        <InquiryDetailModal
          ...
          onClose={() => { setIsModalOpen(false); setSelectedInquiryId(null); }}
