                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-4 pt-3 border-t border-gray-50">
                        <span className="text-[10px] font-bold text-gray-400 uppercase mr-1">Share Lead:</span>
                        <a 
                          href={`https://wa.me/?text=${encodeURIComponent(`Check out this project lead on HousePlanFiles:\n\nTitle: ${lead.title}\nCity: ${lead.city}\nBudget: ${lead.budget}\n\nView leads: https://houseplanfiles.com/leads`)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-[#25D366] text-white p-1.5 rounded-md hover:opacity-90 transition-opacity"
                          title="Share on WhatsApp"
                        >
                          <MessageCircle className="w-3.5 h-3.5" />
                        </a>
                        <a 
                          href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://houseplanfiles.com/leads')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-[#3b5998] text-white p-1.5 rounded-md hover:opacity-90 transition-opacity"
                          title="Share on Facebook"
                        >
                          <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                        </a>
                        <a 
                          href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out project leads on HousePlanFiles!`)}&url=${encodeURIComponent('https://houseplanfiles.com/leads')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-black text-white p-1.5 rounded-md hover:opacity-90 transition-opacity"
                          title="Share on X"
                        >
                           <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        </a>
                        <a 
                          href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('https://houseplanfiles.com/leads')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-[#0077b5] text-white p-1.5 rounded-md hover:opacity-90 transition-opacity"
