...
interface MarketplacePackagesProps {
  packages: Package[];
}
    ...
    >
      {pkg.isPopular && (
        <div className="absolute top-0 w-full bg-orange-500 text-white text-[10px] font-bold py-1 text-center uppercase tracking-widest flex items-center justify-center gap-1 shadow-sm z-20">
          <Star className="w-3 h-3 fill-current" /> Top Seller Choice
        </div>
      )}
...
const MarketplacePackagesSection: React.FC<MarketplacePackagesProps> = ({
  packages,
}) => {
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight">
            Marketplace <span className="text-orange-600">Plans</span>
          </h2>
          <p className="text-sm md:text-lg text-gray-500 mt-2 max-w-2xl mx-auto">
            Boost your product visibility and reach thousands of customers.
          </p>
        </motion.div>
        ...
        <div className="block md:hidden">
          ...
          {hasMoreMobile && (
            <div className="text-center mt-6">
              ...
              >
                {showAllMobile ? "Show Less" : "View All Plans"}
                {showAllMobile ? (
